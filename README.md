...
- **Maximum Drawdown** — capital preservation during downturns
- **Savings Consistency Score** — improvement vs. manual human baseline
...
feature/*  → one branch per task/model
```

Example: `feature/ise-lstm`, `feature/ssi-xgboost`, `feature/finbert-sentiment`

**Workflow:** branch from `develop` → build/train → PR into `develop` → merge to `main` on release.

##  Constraints

- Simulation-only — no live banking (UPI/Net-banking) or brokerage integration
- Signals limited to daily/hourly data granularity (no HFT)
- Historical patterns assumed representative of future behavior (Black-Swan risk acknowledged)

## 📚 References

See [`docs/references.md`](docs/references.md) for the full literature survey and citations.
